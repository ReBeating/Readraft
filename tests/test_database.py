...
def test_database_job_lifecycle(tmp_path: Path):
    ...
    assert not database.complete_analysis(
    ...
    second = database.claim_next_analysis()
    assert second is not None
    database.fail_analysis(
        second["analysis_id"],
        job_id,
        "test failure",
        second["claim_token"],
    ...
    job = database.get_job(user_id, job_id)
    ...
    assert database.retry_failed(user_id, job_id)
