...
长篇上下文的检索取舍与向量化启用门槛见
[《长篇记忆检索架构》](docs/retrieval-architecture.md)。
参考小说如何形成证据化文风画像、怎样安全地反哺原创写作见
[《参考小说分析与文风迁移》](docs/reference-analysis-and-style.md)。
...
- TXT/Markdown 会先生成带边界置信度的分章预览，作者可改名、合并或按光标拆分后再
  确认导入；TXT 正文导出、可校验 `.readraft.zip` 作品归档、完整实例备份与恢复均已提供。
- 参考书分析按结构、事实、叙事、文风和技法分层保存；每层可以独立重试并按内容哈希
  复用。模型判断携带可逐字回查的原文字符区间，全书文风画像只保留定量节奏、跨章
  覆盖率、抽象执行规则和原创性边界，不把参考书原句送进正文写作包。
...
资料，组装本轮相关写作包，再交给写作模型生成完整章节结果。结构化 Planner、记忆提取、
...
Agent 的模型回合可以连续调用多个工具，不设置默认总步数或工具预算。服务端只在重复
调用没有新信息时熔断，并保留权限校验、并发 revision 和可撤回提交等安全边界。全书计划、未来
...
`MODEL_MAX_TOKENS=0` 表示自动模式：对允许省略输出上限的协议不发送该字段，由模型
服务决定单次输出能力；若服务商仍以 length 结束，正文和 Agent 最终回答会从截断处
继续。只有部署者显式填写正整数时，Readraft 才主动下发单次输出额度。工具的
`limit`、`line_count` 与 `max_results` 都是分页大小，不是作品数据总量上限。
正文、章节列表、连续创作、专项任务资源和对话消息都不设默认产品总量配额；完整内容
会持久化到版本或虚拟工作区，模型按需分页读取。仍保留上传/解压大小、浏览器自动暂存、
网络超时、并发 revision 校验与重复无进展熔断，这些只保护服务器和事务，不裁剪作品。
...
小说正文质量另有可重复的 Low / Standard / Max 基准、机器指标和盲评表，见
[写作质量基准](docs/writing-quality-benchmark.md)。

长篇记忆检索另有可重复的精确事实、知情边界、远距离伏笔、因果与作用域回归集，见
[检索架构](docs/retrieval-architecture.md)。运行 `python -m app.retrieval_benchmark`
即可生成 Recall@5、MRR 与越界命中报告。
